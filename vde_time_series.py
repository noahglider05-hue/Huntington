...
print("Fetching VDE data via Yahoo Finance API...")
ticker_data = yf.download('VDE', start='2000-01-01', end='2025-01-01')
...
df_model = pd.concat([df_lagged['VDE'], df_lagged[features]], axis=1)

model = smf.ols('VDE ~ ECON_TREND_PC1 + UNRATE + FEDFUNDS + MCOILWTICO', data=df_model).fit()
...
y = df_model['VDE']
plt.plot(y.index, y, color='#333333', linewidth=2.5, alpha=0.8, label='Actual VDE (Scaled)')
plt.plot(y.index, y_pred, color='#FF5733', linewidth=2, linestyle='--', label='OLS Model Prediction')

plt.title(f'Real Data: VDE (Energy) Price Prediction Model\nAccuracy (R²): {r2:.2f}', fontsize=16, fontweight='bold')
...
plt.savefig('vde_ols_timeseries.png', dpi=300)
print("Done. Graph saved to 'vde_ols_timeseries.png'")